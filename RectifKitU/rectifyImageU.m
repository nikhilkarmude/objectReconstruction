% im1: input image 1
% im2: input image 2
% F: funamental matrix between two images
% ml: points on the left
% mr: points on the right
% rim1: rectified image 1
% rim2: rectified image 2
% TL: homography that rectified im1 to rim2
% TR: homography that rectified im2 to rim2
% bbL: left bounding box
% bbR: right bounding box
function [rim1, rim2, TL, TR, bbL, bbR, err] = rectifyImageU(im1, im2, F, ml, mr)
...
%figure(1)
subplot(2,2,1)
[IL] = im1;%imread(['images/' img_base '0.png'],'png');
image(IL);
axis image
hold on
title('Left image');
...
%figure(2)
subplot(2,2,2)
[IR] = im2;%imread(['images/' img_base '1.png'],'png');
image(IR);
axis image
hold on
title('Right image');
...
%save(['data/' img_base '_T.mat'], 'TL', 'TR', 'bbL', 'bbR');
